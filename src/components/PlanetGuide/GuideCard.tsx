...
const GuideCard: React.FC<GuideCardProps> = ({ title, description, icon }) => {
    return (
        <div className="bg-[rgba(28,28,58,1)] grow text-2xl font-normal w-full px-[23px] py-[27px] rounded-xl max-md:max-w-full max-md:mt-10 max-md:px-5">
            <div className="bg-[rgba(0,0,0,0)] w-full max-w-full text-white pb-[7px] px-[18px]">
                <div className="bg-[rgba(0,0,0,0)] flex items-center px-[21px] py-[5px] max-md:pl-5">
                    {icon && (
                        <img
                            src={icon}
                            alt="Icon"
                            className="aspect-[1] object-contain w-[30px] z-10 shrink-0 mr-4 max-md:-mr-3"
                        />
                    )}
                    <h3>{title}</h3>
                </div>
